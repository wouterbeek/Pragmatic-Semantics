...
public class GraphCoderCombinatorial {
	...
	public void encode(RDFGraph G) {
		...
		int nnamed = G._n_subj2term.size();
		...
		for (int nb = 0; nb < 2; nb++) {
			for (SortedMap<Integer, SortedSet<Integer>> map : (nb == 0 ? G._n_subj2term
			    : G._b_subj2term)) {
		...
		_acc.add("prelim", Codes.universal_nonnegint(nbnodes)); // number of
		// bnodes
		_acc.add("prelim", Codes.universal_nonnegint(npreds)); // number of
		// predicates
		...
		_acc.add("counts", Codes.lgbinomial(tot + 6 - 1, 6 - 1)); // array of
		// link
		// counts
		// per
		// src/dst
		...
		for (int t1 = 0; t1 < 2; t1++) {
			for (int t2 = 0; t2 < 3; t2++) {
				_acc.add("counts",
				    Codes.lgbinomial(sum[t1][t2] + npreds - 1, npreds - 1));
			}
		}

		/*
		 * now that the decoder knows the #links for each src type,dst type,pred
		 * combination, we can transmit the individual links. Literals are handled
		 * separately because on the one hand, we don't need to encode the targets,
		 * as they are transmitted in order in the first part of the code. We do
		 * need to take care that here may be multiple literals for each (subj,pred)
		 * pair.
		 */
		for (int pred = 0; pred < npreds; pred++) {
			for (int t1 = 0; t1 < 2; t1++) {
				for (int t2 = 0; t2 < 2; t2++) {
					// adjacency matrix between objects of type t1 and t2
					// respectively:
					_acc.add("adjacency",
					    Codes.lgbinomial(nsubj[t1] * nsubj[t2], counts[t1][t2][pred]));
				}
				// specify #literals with this pred for each subject
				if (nsubj[t1] > 0)
					_acc.add("literals", Codes.lgbinomial(counts[t1][2][pred] + nsubj[t1]
					    - 1, nsubj[t1] - 1));
			}
		}

		// We used a fixed bnode ordering, however, bnodes can be reordered
		// arbitrarily without
