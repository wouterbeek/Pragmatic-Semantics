...
public class StringTree {
	...
	private boolean _isEndPoint; // whether the path from the root to here
	// represents a complete path
	private ArrayList<StringTree> _children; // set to null until a first child
	// is added
	...
	public StringTree(StringTree parent, String edgeLabelFromParent,
	    boolean isEndPoint) {
	...
	public StringTree lookup(String path) {
		for (StringTree ch : _children) {
			if (path.startsWith(ch._edgeLabelFromParent))
				return ch;
		}
		return null;
	}

	public String getEdgeLabelFromParent() {
		return _edgeLabelFromParent;
	}
	...
	public void add(String path) {
		...
		if (!path.startsWith(child._edgeLabelFromParent)) {
			...
			if (child._edgeLabelFromParent.startsWith(path)) {
				...
				split_at = 1;
				while (path.charAt(split_at) == child._edgeLabelFromParent
				    .charAt(split_at))
					split_at++;
			}
	...
	private void splitLabel(int depth) {
		StringTree new_child = new StringTree(this,
		    _edgeLabelFromParent.substring(depth), _isEndPoint);
		_isEndPoint = false;
	...
	private void addChild(StringTree T) {
		if (_children == null)
			_children = new ArrayList<StringTree>();
		else
			_isSorted = false;
		_children.add(T);
	}
	...
	private void sortChildren() {
		if (!_isSorted) {
			Collections.sort(_children, new Comparator<StringTree>() {
				@Override public int compare(StringTree T1, StringTree T2) {
	...
	/*
	 * Syntax: - output '\t' if this is an internal node AND an endpoint. Then, -
	 * for each child, in alphabetical order of their edge label: - edge label to
	 * that child terminated by new line - recursively output subtree for that
	 * child (depth first) - empty edge label to signal that there are no further
	 * children (i.e., double newline)
	 * 
	 * Note that this method avoids specifying the edge label for the root note,
	 * which is undefined
	 */
	...
	private void getSetRec(Set<String> S, String prefix) {
		...
		if (_children != null) {
			for (StringTree child : _children)
				child.getSetRec(S, prefix + child._edgeLabelFromParent);
		}
	}
