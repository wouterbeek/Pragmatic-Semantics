		...
		switch (nurseBee.type) {
			case 'rdfs5':  // uuu rdfs:subPropertyOf vvv . && vvv rdfs:subPropertyOf xxx . > uuu rdfs:subPropertyOf xxx .
				if (nurseBee.prevEdge != null && nurseBee.prevEdge.id == subPropertyOfURI && nurseBee.passedEdge.id == subPropertyOfURI) {
					if (nurseBee.prevEdge.target == nurseBee.passedEdge.source) {
						found = rdfGraph.newInferredEdge(nurseBee.prevEdge.source, subPropertyOfURI, nurseBee.passedEdge.target, 'rdfs', 'local');
					} else if (nurseBee.prevEdge.source == nurseBee.passedEdge.target) {
						found = rdfGraph.newInferredEdge(nurseBee.passedEdge.source, subPropertyOfURI, nurseBee.prevEdge.target, 'rdfs', 'local');
					}
				}
				break;
			case 'rdfs11': // uuu rdfs:subClassOf vvv .    && vvv rdfs:subClassOf xxx .    > uuu rdfs:subClassOf xxx .
				if (nurseBee.prevEdge != null && nurseBee.prevEdge.id == subClassOfURI && nurseBee.passedEdge.id == subClassOfURI) {
					if (nurseBee.prevEdge.target == nurseBee.passedEdge.source) {
						found = rdfGraph.newInferredEdge(nurseBee.prevEdge.source, subClassOfURI, nurseBee.passedEdge.target, 'rdfs', 'local');
					} else if (nurseBee.prevEdge.source == nurseBee.passedEdge.target) {
						found = rdfGraph.newInferredEdge(nurseBee.passedEdge.source, subClassOfURI, nurseBee.prevEdge.target, 'rdfs', 'local');
					}
				}
				break;
			case 'rdfs9':  // uuu rdfs:subClassOf xxx .    && vvv rdf:type uuu .           > vvv rdf:type xxx .
				if (nurseBee.prevEdge != null && nurseBee.prevEdge.id == subClassOfURI && nurseBee.passedEdge.id == typeURI && nurseBee.prevEdge.source == nurseBee.passedEdge.target) {
					found = rdfGraph.newInferredEdge(nurseBee.passedEdge.source, typeURI, nurseBee.prevEdge.target, 'rdf', 'local');
					...
					found = rdfGraph.newInferredEdge(nurseBee.prevEdge.source, typeURI, nurseBee.passedEdge.target, 'rdf', 'local');
				}
				break;
			case 'rdfs8':
				if (nurseBee.passedEdge != null && nurseBee.passedEdge.id == typeURI && nurseBee.isAt.id == classURI) {
					...
					// found = rdfGraph.newInferredEdge(nurseBee.wasAt.id, subClassOfURI, resourceURI, 'rdfs', 'local');
				}
				break;
			case 'rdfs13':
				if (nurseBee.passedEdge != null && nurseBee.passedEdge.id == typeURI && nurseBee.isAt.id == datatypeURI) {
					...
					// found = rdfGraph.newInferredEdge(nurseBee.wasAt.id, subClassOfURI, literalURI, 'rdfs', 'local');
				}
				break;
			default:
				found = false;
...
function matches(context, pattern, result) {
	...
	*/
	...
	return rdfGraph.newInferredEdge(result.source, result.id, result.target, result.type, 'local');
}